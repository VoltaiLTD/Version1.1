/*
  # Safe Stripe Integration Schema

  1. Tables
    - `stripe_customers` - Maps Supabase users to Stripe customers
    - `stripe_subscriptions` - Stores subscription data
    - `stripe_orders` - Stores one-time payment orders
  
  2. Security
    - Enable RLS on all tables (safely)
    - Add policies for authenticated users to access their own data (safely)
...
-- Safely create policy for stripe_customers
DO $$ BEGIN
...
-- Safely create policy for stripe_subscriptions
DO $$ BEGIN
...
-- Safely create policy for stripe_orders
DO $$ BEGIN
...
-- Create or replace views (these can be safely replaced)
